...
const Single_illness = () => {
  ...
  useEffect(() => {
    const fetchConditionDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/illness/${id}`);
  ...
  if (!condition || !relateHospital.length) {
    return <p>Loading...</p>;
  }
  const handleMap = (id) => {
      ...
      <main>
        <div className="illness_con">
          ...
          <div className="relate_hos">
            <p className="title">
              Exploring the Interconnected Web of Illnesses: A Comprehensive
              Analysis of Health Conditions Spanning Multiple Hospitals
            </p>
