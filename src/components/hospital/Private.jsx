...
const Private = () => {
  ...
  const [filteredHospitalsCount, setFilteredHospitalsCount] = useState(8);
  ...
  useEffect(() => {
    const privateHospitals = hospitalData.filter(
      (hospital) => hospital.hospital_type[0].lable.toLowerCase() === "private"
    );
  ...
  const filteredHospitals = loca_data.filter((hospital) => {
    const isPrivate =
      hospital.hospital_type[0].lable.toLowerCase() === "private";
